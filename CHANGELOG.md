...
-  Added the ability to display the `SleepPageView` after launching the application. Added ability to return to the `SleepPageView` screen after clicking on `Stop Sleeping`. Delete `ContentView`. Stretched background image ignoring `safeArea` - [#91](https://github.com/ios-course/swiftcowboys-team-project/pull/91)
